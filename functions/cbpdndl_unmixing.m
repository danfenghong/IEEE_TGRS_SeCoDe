function [D, Y, GYr, est_S, optinf] = cbpdndl_unmixing(D0, S, Sl, gt, A, HSI, lambda, opt)
...
%   clear D;
  ...
  GYr = real(ifft2(bsxfun(@times,Gf,Yf)));
  GYfs = sum(bsxfun(@times,Gf,Yf),3);
  Jdf = sum(vec(abs(GYfs-Sf).^2))/(2*xsz(1)*xsz(2));
%   clear Yf;
  Jfn = Jdf + lambda*Jl1;
  GY = real(ifft2(permute(GYfs,[1,2,4,3])));
%   Sh = permute(S,[1,2,4,3]);
%   S0 = Sh+Sl;
% [~, aRMSE, armse(k)] = abdrmse(S0, gt, eye(5));

  est_S = abs(GY+Sl);
% est_S_0 = reshape(est_S, [size(est_S,1)*size(est_S,2), size(est_S,3)]);
%   est_S_0 = est_S_0';
%   for i = 1:size(gt,2)
%         aRMSE(i) = sqrt(mean((gt(:,i)-est_S_0(:,i)).^2)); 
%     end
%     armse(1,k) = mean(aRMSE);
%     est_Y = double(A*est_S_0);
%   rrmse(1,k) = XRMSE(est_Y,HSI);
%     rsam(1,k) = XSAM(est_Y,HSI);
    
%   est_S = est_S./repmat(sum(est_S,3),[1,1,size(est_S,3)]);
%   est_S = reshape(est_S, [size(est_S,1)*size(est_S,2), size(est_S,3)]);
%   est_S = est_S';
% %   [~, aRMSE, armse(k)] = abdrmse(est, gt, eye(5));
%     for i = 1:size(gt,2)
%         aRMSE(i) = sqrt(mean((gt(:,i)-est_S(:,i)).^2)); 
%     end
%     armse(2,k) = mean(aRMSE);
%     
%     est_Y = double(A*est_S);
%     rrmse(2,k) = XRMSE(est_Y,HSI);
%     rsam(2,k) = XSAM(est_Y,HSI);
